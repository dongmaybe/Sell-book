   ...
   if(isset($_POST['order_btn'])){//nhập thông tin đơn hàng từ form submit name='order_btn'
      ...
      if(mysqli_num_rows($cart_query) > 0){//tính tổng tiền và số lượng sách
         ...
         if(mysqli_num_rows($order_query) > 0){//đơn hàng trùng thông tin thông báo đã tồn tại
            ...
            mysqli_query($conn, "INSERT INTO `orders`(user_id, name, number, email, method, address, note, total_products, total_price, placed_on) 
            VALUES('$user_id', '$name', '$number', '$email', '$method', '$address', '$note', '$total_products', '$cart_total', '$placed_on')") or die('query failed');
